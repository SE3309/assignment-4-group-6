...
function Login() {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      if (role === "user") {
        // Call User Login API
        const response = await fetch(`http://localhost:3000/login?UserID=${username}&Password=${password}`);
        ...
        const response = await fetch(`http://localhost:3000/artist-login?email=${username}&password=${password}`);
        const data = await response.json();
